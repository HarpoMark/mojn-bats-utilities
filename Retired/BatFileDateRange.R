...
fromFolder <- rchoose.dir(default = "d:/",caption="Folder with wav files, example: F:Bat_2016_corrected")
fromFolder <- gsub("\\","/",fromFolder,fixed=T)
year.list <- c('2018','2019','2020','2021','2022')
...
for (i in listOfFolders){
  ...
  for (s in subs){
    ...
    print (loc_name)
    #"APIS026A"
    listOfWavs <- list.files(s,pattern="\\.wav$",recursive = T,full.names = T,ignore.case = T)
...
df1 <- df %>%
  rename (location=matrix.unlist.list_loc...nrow...len..byrow...T.,drive_wav=matrix.unlist.list_wav...nrow...len..byrow...T.)
df2 <- df1 %>% group_by(location) %>% summarise_at(vars(drive_wav), list(drive_wav = sum))
