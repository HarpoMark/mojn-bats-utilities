...
for (i in listOfFolders){
  print (i)
  listOfWavs <- list.files(i,pattern="\\.JPG$",recursive = F,full.names = T,ignore.case = T)
...
df1 <- df %>%
  rename (deployment=matrix.unlist.list_loc...nrow...len..byrow...T.,JPGCount=matrix.unlist.list_wav...nrow...len..byrow...T.)
df2 <- df1 

st=format(Sys.time(),"%Y%m%d_%H%M")
write.xlsx(df2,file=paste0(fromFolder,"/","JPG_Count_",year,"_",st,".xlsx"),row.names = F)
